...
const Toolbar = ({ tool, setTool, handleColorClick, handleColorChange, color, colorInputRef, elements, undo, redo, clearCanvas }) => {
  ...
  return(
    <div className="w-full h-16 flex mb-6 justify-between px-6">
      ...
      <div className="flex items-center space-x-2">
        <button disabled={elements.length === 0} onClick={undo} className="border-2 py-1 px-2 rounded-md cursor-pointer border-gray-400 hover:text-blue-500"><FaUndo /></button>
        <button disabled={history.length < 1} onClick={redo} className="border-2 py-1 px-2 rounded-md cursor-pointer border-gray-400 hover:text-blue-500"><FaRedo /></button>
        <button onClick={clearCanvas} className="border-2 py-1 px-2 rounded-md cursor-pointer border-gray-400 hover:text-red-500"><FaTrash /></button>
      </div>
    </div>
  );
